// Dump the caller function at cs=168:0x522994..0x522a00
...
function dumpRegion(label, base, len) {
  ...
  for (let i = 0; i < len; i += 16) {
    const bytes = [];
    for (let j = 0; j < 16 && i + j < len; j++) bytes.push(emu.memory.readU8(base + i + j).toString(16).padStart(2, '0'));
    console.log(`  +${(i).toString(16).padStart(3, '0')} (0x${(base+i).toString(16)}): ${bytes.join(' ')}`);
  }
}
dumpRegion('cs=168:0x51fe00..0x51fe80 (function 0x51fe14)', 0x51fe00, 0x80);
