// Trace entry into mixer at cs=168:0x52229b, showing caller regs.
...
cpu.step = function() {
  const eip = cpu.eip >>> 0;
  const cs = cpu.cs;
  if (cs === 0x168 && eip === 0x522299 && hitCount < 3) {
    hitCount++;
    const esp = cpu.reg[4] >>> 0;
    const retAddr = emu.memory.readU32(esp);
    console.log(`\n[MIXER ENTRY #${hitCount}] step=${emu.cpuSteps}`);
    ...
    console.log(`  ESI=0x${(cpu.reg[6]>>>0).toString(16)} EDI=0x${(cpu.reg[7]>>>0).toString(16)} EBP=0x${(cpu.reg[5]>>>0).toString(16)} ESP=0x${esp.toString(16)}`);
    console.log(`  [ESP] return addr = 0x${retAddr.toString(16)}`);
    console.log('  Ring (last 20):');
    for (const r of ring) console.log(`    ${r}`);
  }
  if (ring.length >= RING) ring.shift();
  const bytes = [];
  for (let i = 0; i < 4; i++) bytes.push(emu.memory.readU8((eip + i) >>> 0).toString(16).padStart(2,'0'));
  ring.push(`cs=${cs.toString(16)} eip=0x${eip.toString(16)} bytes=${bytes.join(' ')} eax=0x${(cpu.reg[0]>>>0).toString(16)} edx=0x${(cpu.reg[2]>>>0).toString(16)} ebx=0x${(cpu.reg[3]>>>0).toString(16)}`);
  origStep();
};
...
for (let tick = 0; tick < MAX_TICKS; tick++) {
  if (emu.halted) break;
  emu.tick();
  if (hitCount >= 3 && emu.cpuSteps > 15_000_000) {
    console.log('[STOPPING after enough hits]');
    break;
  }
}
