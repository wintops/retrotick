// Find who calls 0x522994 at the corruption time
...
let hitCount = 0;
cpu.step = function() {
  if (cpu.cs === 0x168 && (cpu.eip >>> 0) === 0x522994 && hitCount < 10) {
    hitCount++;
    const esp = cpu.reg[4] >>> 0;
    const ret = emu.memory.readU32(esp);
    console.log(`[522994 call #${hitCount}] step=${emu.cpuSteps} from ret=0x${ret.toString(16)} EAX=0x${(cpu.reg[0]>>>0).toString(16)} EDX=0x${(cpu.reg[2]>>>0).toString(16)} ECX=0x${(cpu.reg[1]>>>0).toString(16)} EBX=0x${(cpu.reg[3]>>>0).toString(16)}`);
  }
  origStep();
};
...
for (let tick = 0; tick < 200; tick++) {
  if (emu.halted) break;
  emu.tick();
  if (emu.cpuSteps > 18_000_000) break;
}
