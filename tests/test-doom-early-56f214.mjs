// Check [0x56f214] at various steps during DOOM boot
...
// Right after load, before emu.run() is called — memory should have LE content
console.log(`After load() before run():`);
console.log(`  [0x56f210] = 0x${emu.memory.readU32(0x56f210).toString(16)}`);
console.log(`  [0x56f214] = 0x${emu.memory.readU32(0x56f214).toString(16)}`);
console.log(`  [0x56f218] = 0x${emu.memory.readU32(0x56f218).toString(16)}`);
console.log(`  [0x5523c0] = 0x${emu.memory.readU32(0x5523c0).toString(16)}`);

// Check surrounding area
console.log(`\nDump [0x56f200..0x56f240]:`);
for (let i = 0; i < 0x40; i += 16) {
  const bytes = [];
  for (let j = 0; j < 16; j++) bytes.push(emu.memory.readU8(0x56f200 + i + j).toString(16).padStart(2, '0'));
  console.log(`  0x${(0x56f200+i).toString(16)}: ${bytes.join(' ')}`);
}

emu.run();
// Check after the bss init
for (let tick = 0; tick < 30; tick++) {
  if (emu.halted) break;
  emu.tick();
  if (emu.cpuSteps > 250_000) break;
}
console.log(`\nAfter bss init (step ${emu.cpuSteps}):`);
console.log(`  [0x56f210] = 0x${emu.memory.readU32(0x56f210).toString(16)}`);
console.log(`  [0x56f214] = 0x${emu.memory.readU32(0x56f214).toString(16)}`);
console.log(`  [0x56f218] = 0x${emu.memory.readU32(0x56f218).toString(16)}`);
