...
export function App() {
  ...
  // ✅ 自动加载并运行 EXE
  // ==============================
  const autoRunExe = useCallback(async (url: string) => {
    try {
      const res = await fetch(url);
      const buf = await res.arrayBuffer();
      const peInfo = parsePE(buf);
      handleRunExe(buf, peInfo, undefined, url.split('/').pop() || 'auto.exe');
    } catch (e) {
  ...
  useEffect(() => {
    ...
    document.title = focused ? `${focused.title} - WinTops` : 'WinTops';
