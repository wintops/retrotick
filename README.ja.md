...
- **WinHelp（`.hlp`）ビューアー** — クラシック Windows ヘルプファイル向けの Web 再実装：B+ ツリーディレクトリ走査、LZ77 + Hall／旧式フレーズ解凍、HC30 / HCW3.1 / HCW4 のブロックをまたぐトピックチェーン、type-1 / 20 / 22 / 23 / 32 / 35 レコード、セルごとの ParaInfo を伴うテーブルレイアウト、埋め込み `|bm` 画像、ホットスポットオペコード（jump / popup / macro / cross-file）、キーワード検索インデックス
